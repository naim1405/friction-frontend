...
const AdminListPage = () => {
	...
	const { data, isLoading, isFetching, error, refetch } = useGetAdminsQuery(queryParams);
	...
	const errorMessage = useMemo(() => {
		...
		return getErrorMessage(errorData.data ?? null, errorData.message ?? "Unable to load admin list.");
	}, [error]);
			...
			<section className="relative mx-auto w-full max-w-6xl rounded-3xl border border-black/10 bg-white/75 p-4 shadow-lg backdrop-blur-sm sm:p-8">
				<div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
					<div className="space-y-2">
						...
						<h1 className="text-2xl font-semibold text-slate-900 sm:text-3xl">Admin List</h1>
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						<Link href="/create">
							<Button type="button" className="rounded-full bg-slate-900 text-white hover:bg-slate-800">
								Create Admin
				...
				<div className="mt-6 rounded-2xl border border-black/10 bg-white/85 p-4">
					...
					{isLoading ? (
						...
						<div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center">
							...
							<p className="mt-2 text-sm font-semibold text-slate-700">No admins found</p>
							<p className="mt-1 text-xs text-slate-500">Try changing the search or create a new admin.</p>
						</div>
					) : (
								...
								<tbody className="divide-y divide-slate-100">
									{admins.map((admin) => (
										<tr key={admin.id} className="hover:bg-slate-50/80">
											<td className="px-3 py-3 font-medium text-slate-900">{admin.name}</td>
											<td className="px-3 py-3 text-slate-700">{admin.email}</td>
											<td className="px-3 py-3 text-slate-700">{admin.phone}</td>
