      ...
      <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <article className="rounded-[34px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-slate-950 sm:text-4xl">
            Track what you want to finish next
          </h1>
          <p className="mt-4 text-sm leading-7 text-slate-600 sm:text-base">
            This page is ready for backend integration with user-specific task
            progress, saved plans, and contribution history.
          </p>
          ...
          <div className="mt-6 space-y-4">
            {inProgress.map((task) => (
              <Link
                key={task.id}
                href={`/tasks/${task.id}`}
                className="block rounded-[28px] bg-slate-50 p-5"
              >
        ...
        <div className="space-y-6">
          <article className="rounded-[34px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Backend can later return user-specific progress like current step,
              last visited office, saved route, and follow-up reminders.
            </p>
          </article>
          ...
          <article className="rounded-[34px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Suggested future data fields: `draft`, `pending`, `approved`,
              `rejected`, and reviewer notes.
            </p>
          </article>
